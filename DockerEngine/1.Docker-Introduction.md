...
#### 2.5. Run the image to create the container
	...
	docker run -d -p 808:80 hello-world
...
# 3. DOCKER COMMMANDS
	...
	9. docker run <image-name>
		...
		docker run --name container-name -d -p port:port -v hostPath:/container/volume/path image-name:tag
		
		ex-1: docker run -d -p 8080:8080 --name jenkins jenkins/jenkins:latest
		ex-2: docker run -d -p 8080:8080 --name jenkins -v jenkins_home:/var/jenkins_home jenkins/jenkins:latest
		ex-3: create a new container from existing conatiner: docker run --volumes-from <container-id/name> -d -p 8081:8080 --name jenkins2 jenkins/jenkins:latest
